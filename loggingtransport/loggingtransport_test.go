...
`)
	...
	expect.Error(err).Info(info).Not().ToHaveOccurred(t)
...
`)
		...
		logger := func(item *logging.LogItem) {
			...
			expect.Any(item.URL).Info(info).Info(info).ToBe(t, req.URL)
			...
			expect.Error(item.Err).Info(info).Not().ToHaveOccurred(t)
...
func TestLoggingClient_error(t *testing.T) {
	...
	logger := func(item *logging.LogItem) {
		...
		expect.Error(item.Err).ToHaveOccurred(t)
